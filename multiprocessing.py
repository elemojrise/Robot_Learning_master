...
if __name__ == '__main__':
    ...
    eval_env = Monitor(GymWrapper(suite.make(env_id),obs_image)) # Denne lager da standard environment
    ...
    for n_procs in PROCESSES_TO_TEST:
        ...
        if n_procs == 1:
            ...
            train_env = DummyVecEnv([lambda: Monitor(GymWrapper(suite.make(env_id),obs_image))])
        else:
            # Here we use the "fork" method for launching the processes, more information is available in the doc
            # This is equivalent to make_vec_env(env_id, n_envs=n_procs, vec_env_cls=SubprocVecEnv, vec_env_kwargs=dict(start_method='fork'))
            train_env = SubprocVecEnv([make_robosuite_env(env_id, env_options, i+total_procs) for i in range(n_procs)], start_method='fork')
